...
public:
    ...
    //gets hand total, intelligently treats aces as 1 or 11
    int GetTotal() const;
...
int Hand::GetTotal() const
{
    ...
    //if a first card has a value of 0, then the card is face down; return 0
    ...
    // if containsAce && total <= 11, treat ACE as 11
    if (containsAce && total <= 11)
    {
        //add only ten since we already added 1 for ACE
        total += 10;
    }
    return total;
}
...
{
    //create a vector of players from a vector
...
void Game::Play()
{
    ...
    {
        //compare each player still playing house
        for (pPlayer = m_Players.begin(); pPlayer != m_Players.end(); ++pPlayer)
        {
            if (!(pPlayer->IsBusted()))
            {
