...
int main()
{
    ...
    //assigned one pointer to another -- inexpensive assignment
    cout << "Assigning the returned pointer to another pointer.\n";
    string* pStr = ptrToElement(&inventory, 1);
    cout << "Sending the object pointed by new pointer to cout:\n";
    cout << *pStr << "\n\n";
    ...
    //altering the string object through a returned pointed
...
string* ptrToElement(vector<string>* const pVec, int i)
{
    // returns address of the string in position i of vectors that pVec points to
    return &((*pVec)[i]);
}